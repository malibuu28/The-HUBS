...
export default function PersonalDevelopmentView() {
  ...
  const [activeTab, setActiveTab] = useState<'books' | 'bucket' | 'goals' | 'vision'>('books')
  ...
  const [bucketForm, setBucketForm] = useState({ text: '', category: 'Personal' as BucketListItem['category'] })
  ...
  const toggleBucket = (id: string) => {
    update({
      bucketList: bucketList.map(b =>
        b.id === id
          ? { ...b, completed: !b.completed, completedAt: !b.completed ? new Date().toISOString() : undefined }
          : b
      ),
    } as any)
  }
  ...
  const STATUS_STYLES = {
    reading: 'bg-blush/20 text-blush',
    completed: 'bg-moss/20 text-moss',
    wishlist: 'bg-sand/30 text-warm',
  }
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-sand/30">
        ...
        <p className="text-xs text-warm mb-4">Books · Bucket List · Goals · Vision</p>

        <div className="flex gap-1 mb-4 overflow-x-auto pb-1">
          {(['books', 'bucket', 'goals', 'vision'] as const).map(tab => (
            <button
              key={tab}
                  ...
                  {currentlyReading.map(book => (
                    <div key={book.id} className="bg-cream/50 rounded-xl p-3 border border-sand/20">
                      <div className="flex items-start justify-between">
                        <div>
                          <p className="text-sm font-medium text-deeper">{book.title}</p>
                          <p className="text-xs text-warm">{book.author}</p>
                        </div>
                        <button
                          onClick={() => updateBookStatus(book.id, 'completed')}
                          className="text-xs bg-moss/20 text-moss px-2 py-1 rounded-lg"
                        >Done ✓</button>
                      </div>
                    </div>
                  ))}
          ...
          <div className="space-y-3">
              ...
              {bucketList.map(item => (
                ...
                >
                  <div className={`w-5 h-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center ${
                    item.completed ? 'bg-moss border-moss' : 'border-sand'
                  }`}>
                    {item.completed && <span className="text-white text-xs">✓</span>}
                  </div>
                  <div className="flex-1">
                    <p className={`text-sm ${item.completed ? 'text-moss line-through' : 'text-deeper'}`}>
                      {item.text}
                    </p>
                    <p className="text-xs text-warm">{item.category}</p>
                  </div>
                </button>
              ))}
            </div>
            <div className="bg-cream/50 rounded-xl p-3 space-y-2">
              ...
              <select
                ...
                onChange={e => setBucketForm({ ...bucketForm, category: e.target.value as any })}
              >
        ...
        {activeTab === 'goals' && (
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <input
                placeholder="Add a personal development goal..."
        ...
        {activeTab === 'vision' && (
          <div className="space-y-3">
            ...
            <textarea
              placeholder="Who do you want to become this year? What skills, habits, and growth are you committed to?"
              className="w-full mt-2 border border-sand/30 rounded-xl px-3 py-2 text-sm text-deeper bg-cream/50 focus:outline-none"
              rows={5}
              value={yearlyVisionPD}
              onChange={e => {
                setYearlyVisionPD(e.target.value)
                update({ yearlyVisionPD: e.target.value } as any)
              }}
            />
          </div>
        )}
