...
export default function BusinessView() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'income' | 'goals' | 'debt' | 'products'>('overview')
  ...
  const toggleGoal = (id: string) => {
    update({
      businessGoals: businessGoals.map(g =>
  ...
  const SOURCE_LABELS = {
    salary: { label: 'Salary', icon: '💼', color: 'text-deeper' },
    hair: { label: 'Hair Business', icon: '✂️', color: 'text-blush' },
    other: { label: 'Other', icon: '💰', color: 'text-sage' },
  }
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-sand/30">
        ...
        <p className="text-xs text-warm mb-4">Income · Debt · Products · Goals</p>

        <div className="flex gap-1 mb-4 overflow-x-auto pb-1">
          {(['overview', 'income', 'goals', 'debt', 'products'] as const).map(tab => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`flex-shrink-0 px-3 py-2 rounded-xl text-xs font-medium transition-all capitalize ${
                activeTab === tab
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-cream/50 rounded-xl p-3 border border-sand/20">
                <p className="text-xs text-warm mb-1">Total Income</p>
                <p className="text-xl font-light text-deeper">${totalIncome.toLocaleString()}</p>
              </div>
              <div className="bg-cream/50 rounded-xl p-3 border border-sand/20">
                <p className="text-xs text-warm mb-1">Hair Business</p>
                <p className="text-xl font-light text-blush">${hairIncome.toLocaleString()}</p>
              </div>
              <div className="bg-cream/50 rounded-xl p-3 border border-sand/20">
                <p className="text-xs text-warm mb-1">Salary</p>
                <p className="text-xl font-light text-sage">${salaryIncome.toLocaleString()}</p>
              </div>
              <div className="bg-cream/50 rounded-xl p-3 border border-sand/20">
                <p className="text-xs text-warm mb-1">Q Goals</p>
                <p className="text-xl font-light text-moss">
                  {currentQuarterGoals.filter(g => g.completed).length}/{currentQuarterGoals.length}
                </p>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'income' && (
          <div className="space-y-3">
              ...
              {income.slice(-5).reverse().map(entry => (
                <div key={entry.id} className="flex items-center justify-between bg-cream/50 rounded-xl p-3 border border-sand/20">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-sm text-deeper">{SOURCE_LABELS[entry.source].label}</p>
                      <p className="text-xs text-warm">{entry.date} {entry.note && `· ${entry.note}`}</p>
                    </div>
                  </div>
            ...
            <div className="bg-cream/50 rounded-xl p-3 space-y-2">
              ...
              >
                {(Object.keys(SOURCE_LABELS) as IncomeEntry['source'][]).map(s => (
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              {currentQuarterGoals.map(goal => (
                ...
                >
                  ...
                  <span className={`text-sm ${goal.completed ? 'text-moss line-through' : 'text-deeper'}`}>
                    {goal.text}
                  </span>
                </button>
              ))}
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 border border-sand/30 rounded-xl px-3 py-2 text-sm text-deeper bg-cream/50"
                value={goalText}
